...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,const vector< vector<tDetection> > &detections, bool compute_aos, vector<double> &precision, vector<double> &aos, DIFFICULTY difficulty, int top_stats) {
  ...
  if (top_stats>0 && difficulty==MODERATE && current_class+1==top_stats) {
    ...
    if (top_stats>0) {
      system(("mkdir " + top_dir).c_str());
      system(("mkdir " + top_dir + "/tp").c_str());
      system(("mkdir " + top_dir + "/fp").c_str());
      system(("mkdir " + top_dir + "/fn").c_str());
    }
...
bool eval(FILE * file_names, string gt_dir, string result_dir, Mail* mail, int top_stats){
  ...
  if(eval_car){
    ...
    if (top_stats==0 || top_stats==1) {
      if(   !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,top_stats)
         || !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,top_stats)
         || !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,top_stats)){
  ...
  if(eval_pedestrian){
    ...
    if (top_stats==0 || top_stats==2) {
      if(   !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,top_stats)
         || !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,top_stats)
         || !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,top_stats)){
  ...
  if(eval_cyclist){
    ...
    if (top_stats==0 || top_stats==3) {
      if(   !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,top_stats)
         || !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,top_stats)
         || !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,top_stats)){
